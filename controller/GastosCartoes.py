...
async def addExpensesCard(item: SchemasP.Gastos_Cartao_Mes, session: Session = Depends(get_session), user: Cadastro_Users = Depends(get_current_user)): #Aqui se chamaria a classe, e o nome do classe dentro da classe, para pegar os valores e fazer um objeto
    return await addExpensesCardMonthService(item, session, user)

@router.patch("/gastos_cartao/adicionar_valores_dias")
async def addExpensesCardDay(item: SchemasP.Gastos_Cartao, session: Session = Depends(get_session), user: Cadastro_Users = Depends(get_current_user)):
