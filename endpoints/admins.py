...
def addItem(item:schemas.Admins, session: Session = Depends(get_session), user: Cadastro_Users = Depends(get_current_user)): #Aqui se chamaria a classe, e o nome do classe dentro da classe, para pegar os valores e fazer um objeto
    if not user.is_admin:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
    ...
    #newId = len(fakeDataBase.keys()) + 1 #Pegando o tamanho do fakedatabase e adicionando o valor de +1 para que o próximo item que for adicionado seja na próxima key
    item = models.Cadastro_Admins(nome = item.nome, email = item.email, senha = item.senha)
    ...
    #return fakeDataBase
    return {f"Olá {user.username}, adicionando o admin: {item}"}
