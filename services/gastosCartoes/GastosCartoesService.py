...
async def addExpensesCardMonthService(item: schemasP.Gastos_Cartao_Mes, session: Session = Depends(get_session), user: Cadastro_Users = Depends(get_current_user)):
    try:
        ...
        item = modelsP.Gastos_Cartao_Mes(
        ...
        produtoMes = session.query(modelsP.Gastos_Cartao_Mes).filter(modelsP.Gastos_Cartao_Mes.mes == item.mes).first()
        ...
        if produtoMes:
            ...
            logging.info("Valor de gasto aleatório total atualizado para o mês com sucesso.")
            return f"Olá, {user.username}, o Valor de gasto aleatório total foi atualizado para o mês desejado com sucesso!", item
        else:
            #Criando o novo produto no banco
            novo_produto_mes = modelsP.Gastos_Cartao_Mes(
                mes=item.mes.capitalize(),
                valor=item.valor,
            )
...
async def addDaysExpensesCardService(item: schemasP.Gastos_Cartao, session: Session = Depends(get_session), user: Cadastro_Users = Depends(get_current_user)):
    try:
        if not user.is_admin:
...
async def getDaysExpensesCardServices(mes: str, session: Session = Depends(get_session), user: Cadastro_Users = Depends(get_current_user)): #Pegando os valores do banco de dados, Depends do get_session. E verificando se o usuário está logado, com o get_current_user
    try:
        mes = session.query(modelsP.Gastos_Cartao_Cad).filter_by(mes=mes.capitalize()).all()
        if mes is None:
...
async def getExpensesCardServices(session: Session = Depends(get_session), user: Cadastro_Users = Depends(get_current_user)): #Pegando os valores do banco de dados, Depends do get_session. E verificando se o usuário está logado, com o get_current_user
    try:
...
async def deleteExpensesCardMonthService(mes:str, session: Session = Depends(get_session), user: Cadastro_Users = Depends(get_current_user)): #Aqui se chamaria a classe, e o nome do classe dentro da classe, para pegar os valores e fazer um objeto
    try: 
        if not user.is_admin:
            raise HTTPException(
        ...
        itemObject = session.query(modelsP.Gastos_Cartao_Mes).filter_by(mes=mes.capitalize()).first() #Pegando o valor que foi passado pelo nome, e procurando no bnaco pra ver se existe algo com esse nome
        if itemObject is None:
