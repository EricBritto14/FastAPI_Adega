...
async def addExpensesMonthService(item: schemasP.Gastos_Aleatorios_Mes, session: Session = Depends(get_session), user: Cadastro_Users = Depends(get_current_user)):
    try:
        if not user.is_admin:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Você não tem permissão para adicionar valores de gastos aos mêses!")
        
        #Criando um objeto para manipular e apenas retornar a % de ganho em cima do produto em _valor_venda
        item = modelsP.Gastos_Aleatorios_Mes(
...
async def addDaysExpensesService(item: schemasP.Gastos_Aleatorios, session: Session = Depends(get_session), user: Cadastro_Users = Depends(get_current_user)):
    try:
        if not user.is_admin:
        ...
        produtoMes = (
            session.query(modelsP.Gastos_Aleatorios_Cad)
            .filter(
                modelsP.Gastos_Aleatorios_Cad.dia == item.dia,
                modelsP.Gastos_Aleatorios_Cad.mes == item.mes,
                modelsP.Gastos_Aleatorios_Cad.motivo == item.motivo
            )
            .first()
        )
        ...
        if produtoMes:
            ...
            session.commit()
            session.refresh(produtoMes)
            logging.info("Valor de gasto aleatório atualizado para o(s) dia(s) com sucesso.")
            return f"Olá, {user.username}, o valor de gasto aleatório para o(s) dia(s) desejado(s) foi atualizado com sucesso!", item
        else:
            #Criando o novo produto no banco
            novo_produto_dia_venda = modelsP.Gastos_Aleatorios_Cad(
                dia=item.dia,
                valor=item.valor,
                mes=item.mes,
                motivo=item.motivo
            ...
            logging.info("Valor de gasto aleatório adicionado para o(s) dia(s) com sucesso.")
            return f"Olá, {user.username}, o valor de gasto aleatório para o(s) dia(s) desejado(s) foi adicionado!", item
    except HTTPException as e:
        session.rollback()
        raise e
    except Exception as e:
...
async def getDaysExpensesServices(mes: str, session: Session = Depends(get_session), user: Cadastro_Users = Depends(get_current_user)): #Pegando os valores do banco de dados, Depends do get_session. E verificando se o usuário está logado, com o get_current_user
    try:
        mes = session.query(modelsP.Gastos_Aleatorios_Cad).filter_by(mes=mes.capitalize()).all()
        if not mes:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Nenhum valor de gasto existente neste mês!")
        else:
...
async def getExpensesServices(session: Session = Depends(get_session), user: Cadastro_Users = Depends(get_current_user)): #Pegando os valores do banco de dados, Depends do get_session. E verificando se o usuário está logado, com o get_current_user
    try:
...
async def deleteExpensesMonthService(mes:str, session: Session = Depends(get_session), user: Cadastro_Users = Depends(get_current_user)): #Aqui se chamaria a classe, e o nome do classe dentro da classe, para pegar os valores e fazer um objeto
    try: 
        if not user.is_admin:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Você não tem permissão para deletar valores de gasto!"
            )
        ...
        itemObject = session.query(modelsP.Gastos_Aleatorios_Mes).filter_by(mes=mes.capitalize()).first() #Pegando o valor que foi passado pelo nome, e procurando no bnaco pra ver se existe algo com esse nome
        if not itemObject:
            raise HTTPException(status.HTTP_400_BAD_REQUEST, detail=f"Valor de gasto no mês: {mes}, não encontrado")
        id = itemObject.idMes
        session.delete(itemObject) #comitando a mudança
        session.commit() #Comitando as mudanças
        session.close() #Fechando o banco
        return f"Olá {user.username}, o valor de gasto {mes} e id {id}, foi deletado!", itemObject
    except Exception as e:
        session.rollback() #Session rollback serve para que se cair na exception, garantir que não faça nada no banco. Então rollback para garantir que não deu nada, antes de dar erro.
        raise HTTPException(status_code=e.status_code, detail=str(e))
